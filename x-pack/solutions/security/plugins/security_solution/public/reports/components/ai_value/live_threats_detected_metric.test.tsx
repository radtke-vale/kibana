...
describe('LiveThreatsDetectedMetric', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('calls useSpaceId hook', () => {
    render(<LiveThreatsDetectedMetric {...defaultProps} />);
  ...
  it('passes correct timerange to VisualizationEmbeddable', () => {
    ...
    render(<LiveThreatsDetectedMetric from={customTimeRange.from} to={customTimeRange.to} />);
  ...
  it('memoizes the component correctly', () => {
    const { rerender } = render(<LiveThreatsDetectedMetric {...defaultProps} />);
    const initialCallCount = (VisualizationEmbeddable as unknown as jest.Mock).mock.calls.length;
    rerender(<LiveThreatsDetectedMetric {...defaultProps} />);
    expect((VisualizationEmbeddable as unknown as jest.Mock).mock.calls.length).toBe(
      initialCallCount
    );
    rerender(
      <LiveThreatsDetectedMetric from="2023-02-01T00:00:00.000Z" to="2023-02-28T23:59:59.999Z" />
    );
    expect((VisualizationEmbeddable as unknown as jest.Mock).mock.calls.length).toBe(
      initialCallCount + 1
    );
